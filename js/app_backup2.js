...
function setFeatureStateAll(iso3, state) {
  if (!iso3) return;
  ["my-countries", "tiny-country-expanded"].forEach((src) => {
...
map.on("load", async () => {
  ...
  (geojson.features || []).forEach((f) => {
    ...
    const LUX_AREA = 2.6e9; // ~2586 km²
    const isIndependent = !allowedIndependentSet || allowedIndependentSet.has(iso3);
    if (isIndependent && (isIslandState || area <= LUX_AREA)) {
      f.properties.outlineTarget = true;
    }
  ...
  // Outline voor eilandstaten en zeer kleine onafhankelijke landen (<= Luxemburg)
  map.addLayer({
    id: "outline-small-islands",
    type: "line",
    source: "my-countries",
    filter: ["==", ["get", "outlineTarget"], true],
    paint: {
      "line-color": "#0f172a",
      "line-width": 1.8,
      "line-blur": 0.4,
  ...
  if (!feats || !feats.length) {
    if (quizStage === "country") {
      ...
      if (currentCountryTarget) {
        const name = iso3ToName.get(currentCountryTarget) || currentCountryTarget;
        setQuizText(`Target country: ${name}`, "");
      } else {
        setQuizText("Target: –", "");
      }
    } else {
    ...
    quizStageSelect.addEventListener("change", () => {
      ...
      if (countriesGeojson) {
        (countriesGeojson.features || []).forEach((f) => {
          const iso = f.properties["ISO3166-1-Alpha-3"];
          if (iso) {
            setFeatureStateAll(iso, { guessed: false, feedback: null, isSpecialLight: null, hatched: null, isHatched: null });
          }
        });
      }
